...
def convert_examples_to_interactive_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for triple in example.triples:
            ...
            # NER标签转换
            label_id_ner = [label_map_ner[i] for i in lable_ner]
            label_ids = OutputExample(h=subject_ids, r=label_map_seq[relation], t=object_ids)
...
def openue_data_collator_ner(features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
    # 读取ner的label
    max_length = [len(f.input_ids) for f in features]
    max_length = max(max_length)
    ...
    for f in features:
        ...
        f.label_ids_ner = f.label_ids_ner + add_special  # 补0, 这里仅仅为了补齐的最长长度, loss计算中有mask存在会被忽略
        # 处理seq label的格式
